...
#for ligand element N and O to protein S or ligand S to protein N and O
...
found_no_hbonds = []
with open("hbonds_statistic_PDB_S_to_NO.csv","w") as f:
  ...
  for pdb in pdb_file_list:
    if pdb: #如果不是空值的话
      ...
      #for lig s and pro n o
      selection = "polymer.protein and e. n+o" #蛋白重原子
      ...
      resi_atom, lig_atom, distances = list_hb(selection, selection2, cutoff=3.5,angle=55,mode=1,hb_list_name='hbonds') #这里的角度就相当于125度
      ...
      #for lig o+n and pro s
      selection = "polymer.protein and e. s" #蛋白重原子
      selection2 = "organic and e. n+o" #残基重原子
      cmd.delete("all")
      cmd.load(path)
      # cmd.select("sele1","organic and e. n+o")
      lig_heavy_atoms = atom = cmd.count_atoms("organic")
